...
class analytical_crystal(_temp,_ThermalParameters):
    ...
    def __post_init__(self):
        ...
        if not self.unitless:
            ...
            self.ur = np.linspace(0,end,10000)
    ...
    def get_analytical_solution(self):
        ...
        def model(t,vars, r, maxn):
            """System of the ODEs that describe the filling and fading 
            of the traps in the crystal."""

            n = vars[0] 
            T = self.Tat(t)
            fill = (self.D_dot/self.D0)*(maxn-n)
            fade = self._fade(T,r)*n
            dn_dt = fill-fade
        ...
        for i in range(len(self.ur)):
            ...
            if self.unitless:
                ...
                solution = solve_ivp(model, (0, self.duration), [0], args=(r,self.ur_weights[i]), t_eval=self.time_steps, method = 'Radau',dense_output=True)
                res= (solution.y[0]/self.ur_weights[i])
